...
gaussian <- gaussian |>
  filter(test %in% c("gaussian_mu_test")) |>
  select(test, alt, param = mu, pvalue) |>
  bind_rows(gaussian |>
    filter(test %in% c("gaussian_variance_test")) |>
    select(test, alt, param = variance, pvalue))

log_normal <- readRDS("results/log_normal_type_one.rds")
log_normal <- log_normal |>
  filter(test %in% c("log_normal_mu_test")) |>
  select(test, alt, param = mu, pvalue) |>
  bind_rows(log_normal |>
    filter(test %in% c("log_normal_variance_test")) |>
    select(test, alt, param = variance, pvalue))
...
gamma <- gamma |>
  filter(test %in% c("gamma_rate_test")) |>
  select(test, alt, param = rate, pvalue) |>
  bind_rows(
    gamma |>
      filter(test %in% c("gamma_scale_test")) |>
      select(test, alt, param = scale, pvalue),
    gamma |>
      filter(test %in% c("gamma_shape_test")) |>
...
beta <- beta |>
  filter(test %in% c("beta_shape1_test")) |>
  select(test, alt, param = shape1, pvalue) |>
  bind_rows(beta |>
    filter(test %in% c("beta_shape2_test")) |>
...
invGauus <- invGauus |>
  filter(test == "inverse_gaussian_mu_test") |>
  select(test, alt, param = mu, pvalue) |>
  bind_rows(
    invGauus |>
      filter(test == "inverse_gaussian_shape_test") |>
      select(test, alt, param = shape, pvalue),
    invGauus |>
      filter(test == "inverse_gaussian_dispersion_test") |>
...
empLikVar <- readRDS("results/empirical_variance_type_one.rds")
empLikVar <- empLikVar |>
  select(test, alt, param = variance, pvalue)

# Excluding empirical variance and empirical quantile text
# Both have to throw away samples because optimization problem cannot
# be solved for some samples. Throwing away some samples makes the
# sampling process bias.
typeI <- bind_rows(
  gaussian,
  log_normal,
  gamma,
...
For all three alternative hypotheses, dots are near the red line for asymptotic p values below .20. All tests are well calibrated over the entire range of asymptotic p values.
...
plot_graph <- function(df, alt_arg) {
  ...
  graph <- df |>
    filter(alt == alt_arg) |>
    mutate(test = str_replace(test, "_test", "")) |>
...
gaussian <- gaussian |>
  filter(test %in% c("gaussian_mu_one_way_test")) |>
  select(test, alt, param = mu, pvalue) |>
  bind_rows(gaussian |>
    filter(test %in% c("gaussian_variance_one_way_test")) |>
    select(test, alt, param = variance, pvalue))

log_normal <- readRDS("results/log_normal_type_one_one_way.rds")
log_normal <- log_normal |>
  filter(test %in% c("log_normal_mu_one_way_test")) |>
  select(test, alt, param = mu, pvalue) |>
  bind_rows(log_normal |>
    filter(test %in% c("log_normal_variance_one_way_test")) |>
    select(test, alt, param = variance, pvalue))
...
gamma <- gamma |>
  filter(test %in% c("gamma_rate_one_way_test")) |>
  select(test, alt, param = rate, pvalue) |>
  bind_rows(
    gamma |>
      filter(test %in% c("gamma_scale_one_way_test")) |>
      select(test, alt, param = scale, pvalue),
    gamma |>
      filter(test %in% c("gamma_shape_one_way_test")) |>
      select(test, alt, param = shape, pvalue)
  )
...
empLikeVar <- readRDS("results/empirical_variance_type_one_one_way.rds")
empLikeVar <- empLikeVar |>
  select(test, alt, param = variance, pvalue)
...
plot_graph <- function(df, alt_arg) {
  ...
  graph <- df |>
    filter(alt == alt_arg) |>
    mutate(test = str_replace(test, "_one_way_test", "")) |>
    ggplot(aes(x = pvalue, emperical_pvalue)) +
    geom_point(alpha = 0.3, shape = ".") +
    stat_function(fun = identity, color = "darkred") +
    scale_x_continuous(breaks = seq(0, 1, .2), labels = drop_leading_zero) +
